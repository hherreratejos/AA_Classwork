...
def all_vowel_pairs(words)
    ...
    words.each_with_index do |w1, i1|
        words.each_with_index do |w2, i2|
            if i2 > i1 && (vowel_count(w1, w2).sort == ["a","e", "i", "o", "u"])
                result << "#{w1} #{w2}"
            end
        end
    end
...
def vowel_count(word1, word2)
    ...
    word2.each_char do |char1|
        if "aeiou".include?(char1) && !vowels.include?(char1)
            vowels << char1
        end
    end
    vowels
...
def find_bigrams(str, bigrams)
    new_bigrams = []
    bigrams.each do |big|
        new_bigrams << big if str.include?(big)
    end
    new_bigrams
end
